...
// old version, going to remove
#include <hardware/Camera/Camera.h>
#include <hardware/Camera/Undistorter.h>

// new version, recommand
#include <hardware/Camera/Cameras.h>
#include <hardware/Camera/undistorter.h>

#ifdef HAS_VIKIT
// the vikit camera class used by svo .etc, compiled for comparation
...
bool CameraTest(pi::hardware::Camera camera)
{
    ...
    vec_cam.resize(times);
    vec_re.resize(times);
    /// Use the same data with the old version
//    vec_p.resize(times);
//    for(int i=0;i<times;i++)
//    {
//        double rdx=pi::Random::RandomValue<double>()*camera.width();
...
int main(int argc,char** argv)
{
    ...
    // apply scale to the camera model, now just pinhole
